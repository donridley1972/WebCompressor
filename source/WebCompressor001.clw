   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('WEBCOMPRESSOR001.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('WEBCOMPRESSOR002.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('WEBCOMPRESSOR003.INC'),ONCE        !Req'd for module callout resolution
                     END
...
ViewPosition           STRING(1024)                   !Entry's view position
                     END
Window               WINDOW('WebCompressor'),AT(,,464,264),FONT('Segoe UI',10),RESIZE,AUTO,ICON('AppIcon.ico'), |
  GRAY,SYSTEM,TIMER(5),IMM
                       BUTTON,AT(441,240,20,15),USE(?Close),LEFT,ICON('Close1.ico')
                       IMAGE('Logo.png'),AT(2,1,260,45),USE(?IMAGE1)
                       SHEET,AT(121,49,341,172),USE(?SHEET2)
                         TAB('Source'),USE(?SourceTab)
                           SHEET,AT(124,64,334,152),USE(?FilesSheet)
                             TAB('Javascript Files'),USE(?SourceJavascriptTab)
                               LIST,AT(130,80,319,128),USE(?SourceJavascriptList),HVSCROLL,FORMAT('1020L(2)|M~Files~@s255@'), |
  FROM(SourceJSQ)
                             END
                             TAB('CSS Files'),USE(?SourceCssTab)
                               LIST,AT(130,80,319,128),USE(?SourceCssList),HVSCROLL,FORMAT('1020L(2)|M~Files~@s255@'),FROM(SourceCssQ)
                             END
                           END
                         END
                         TAB('Destination'),USE(?TAB3)
                           SHEET,AT(124,64,334,152),USE(?DestinationFilesSheet)
                             TAB('Javascript Files'),USE(?DestinationJavascriptTab)
                               LIST,AT(130,80,319,128),USE(?DestinationJavascriptList),VSCROLL,FORMAT('1020L(2)|M~Files~@s255@'), |
  FROM(DestJsQ)
                             END
                             TAB('CSS Files'),USE(?DestinationCSSTab)
                               LIST,AT(130,80,319,128),USE(?DestinationCSSList),HVSCROLL,FORMAT('1020L(2)|M~Files~@s255@'), |
  FROM(DestCssQ)
                             END
                           END
                         END
                       END
                       LIST,AT(3,49,114,171),USE(?List),LEFT(2),VSCROLL,FORMAT('400L(2)|M~Description~@s100@'),FROM(Queue:Browse), |
  IMM
                       BUTTON,AT(53,224,19,12),USE(?Insert),ICON('Insert1.ico')
                       BUTTON,AT(75,224,19,12),USE(?Change),ICON('Edit1.ico')
                       BUTTON,AT(99,224,19,12),USE(?Delete),ICON('Delete1.ico')
                       BUTTON('Compress'),AT(411,223),USE(?CompressBtn)
                     END
        ...
        Of MyState:WaitOnThreads
            If CompressionStarted = TRUE
                If Records(ThreadQ) = 0
                    ...
                    DIRECTORY(DestJsQ, Clip(Pro:JavascriptDestination) & '\*.js', ff_:NORMAL)
                    DIRECTORY(DestCssQ, Clip(Pro:CSSDestination) & '\*.css', ff_:NORMAL)
                    Message('Done!')
                END
            End      
        End
