   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('WEBCOMPRESSPR002.INC'),ONCE        !Local module procedure declarations
                     END
...
QuickWindow          WINDOW('Form Projects'),AT(,,366,127),FONT('Segoe UI',10,,FONT:regular,CHARSET:DEFAULT),RESIZE, |
  AUTO,CENTER,ICON('AppIcon.ico'),GRAY,HLP('UpdateProjects'),SYSTEM,IMM
                       BUTTON('&OK'),AT(209,106,49,14),USE(?OK),LEFT,ICON('WAOK.ICO'),DEFAULT,FLAT,MSG('Accept dat' & |
  'a and close the window'),TIP('Accept data and close the window')
                       BUTTON('&Cancel'),AT(261,106,49,14),USE(?Cancel),LEFT,ICON('WACANCEL.ICO'),FLAT,MSG('Cancel operation'), |
  TIP('Cancel operation')
                       BUTTON('&Help'),AT(314,106,49,14),USE(?Help),LEFT,ICON('WAHELP.ICO'),FLAT,MSG('See Help Window'), |
  STD(STD:Help),TIP('See Help Window')
                       ENTRY(@s16),AT(121,2,68,10),USE(Pro:PKGuid),LEFT(2)
                       PROMPT('PKG uid:'),AT(5,2),USE(?Pro:PKGuid:Prompt),TRN
                       PROMPT('Description:'),AT(5,16),USE(?Pro:Description:Prompt),TRN
                       ENTRY(@s100),AT(121,16,226,10),USE(Pro:Description),LEFT(2)
                       PROMPT('Source Javascript:'),AT(5,31),USE(?Pro:SourceJavascript:Prompt),TRN
                       ENTRY(@s255),AT(121,31,226,10),USE(Pro:SourceJavascript),LEFT(2)
                       PROMPT('Source CSS:'),AT(5,45),USE(?Pro:SourceCSS:Prompt),TRN
                       ENTRY(@s255),AT(121,45,226,10),USE(Pro:SourceCSS),LEFT(2)
                       PROMPT('Javascript Destination:'),AT(5,59),USE(?Pro:JavascriptDestination:Prompt),TRN
                       ENTRY(@s255),AT(121,59,226,10),USE(Pro:JavascriptDestination),LEFT(2)
                       PROMPT('CSS Destination:'),AT(5,72),USE(?Pro:CSSDestination:Prompt),TRN
                       ENTRY(@s255),AT(121,72,226,10),USE(Pro:CSSDestination),LEFT(2)
                       PROMPT('Javascript gZip Destination:'),AT(5,87),USE(?Pro:JavascriptGzipDestination:Prompt), |
  TRN
                       ENTRY(@s255),AT(121,87,226,10),USE(Pro:JavascriptGzipDestination),LEFT(2)
                       BUTTON,AT(351,31,12,11),USE(?LookupFile),ICON('Search1.ico')
                       BUTTON,AT(351,45,12,11),USE(?LookupFile:2),ICON('Search1.ico')
                       BUTTON,AT(351,58,12,11),USE(?LookupFile:3),ICON('Search1.ico')
                       BUTTON,AT(351,72,12,11),USE(?LookupFile:4),ICON('Search1.ico')
                       BUTTON,AT(351,87,12,11),USE(?LookupFile:5),ICON('Search1.ico')
                     END
...
ReturnValue          BYTE,AUTO
  ...
  SELF.AddHistoryField(?Pro:PKGuid,1)
  ...
  SELF.AddHistoryField(?Pro:JavascriptGzipDestination,7)
  ...
  IF SELF.Request = ViewRecord                             ! Configure controls for View Only mode
    ...
    ?Pro:JavascriptGzipDestination{PROP:ReadOnly} = True
    DISABLE(?LookupFile)
    DISABLE(?LookupFile:2)
    DISABLE(?LookupFile:3)
    DISABLE(?LookupFile:4)
    DISABLE(?LookupFile:5)
  END
  ...
  FileLookup11.WindowTitle='Javascript gZip Destination'
  SELF.SetAlerts()
  RETURN ReturnValue
...
Looped BYTE
  ...
  ReturnValue = PARENT.TakeAccepted()
    ...
    OF ?LookupFile:5
      ThisWindow.Update()
      Pro:JavascriptGzipDestination = FileLookup11.Ask(1)
      DISPLAY
    END
    RETURN ReturnValue
  END
